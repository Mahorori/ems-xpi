...
{
	formatted_packet_grammar(COutPacket* oPacket, BOOL bHide) : pPacket(oPacket), bHidden(bHide), formatted_packet_grammar::base_type(start)
	{
		QUOTED_STRING_ %= boost::spirit::qi::lexeme['"' >> *(boost::spirit::ascii::char_ - '"') >> '"'];
		ARRAY_ %= '[' >> boost::spirit::qi::lexeme[+BYTE_] >> ']';

		start =
			WORD_[boost::phoenix::bind(&FormattedInject::DoInitPacket, pPacket, bHidden, boost::spirit::_1)]
			>>
