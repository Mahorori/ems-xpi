...
VOID CSettingsPacket::OnUpdateOpcode(__in WORD wOpcode)
{
	HWND hListview = GetDlgItem(m_hDialog, IDC_OPCODELIST);
	...
	if (iIndex == -1)
	{
		...
		if (FAILED(StringCchPrintfW(wszOpcode, _countof(wszOpcode), L"%04X", wOpcode)))
			return;
...
VOID CSettingsPacket::ListviewCustomDraw(__inout LPNMLVCUSTOMDRAW lpnmlvcd)
{
	...
	case (CDDS_SUBITEM | CDDS_ITEMPREPAINT) :
		...
		if (lpnmlvcd->iSubItem == LVC_OPCODE)
			lpnmlvcd->clrText = (*pOpcodeInfo)[(WORD)lpnmlvcd->nmcd.lItemlParam].crColor;
		else
...
INT CALLBACK CSettingsPacket::ListviewSort(__in LPARAM lParam1, __in LPARAM lParam2, __in LPARAM lParamSort)
{
	...
	else if (Sort == LVS_ALIAS_ASC || Sort == LVS_ALIAS_DESC)
	{
		...
		if (Sort == LVS_ALIAS_ASC)
			return lstrcmpiW((*pOpcodeInfo)[(WORD)lParam1].wszAlias, (*pOpcodeInfo)[(WORD)lParam2].wszAlias);
		else // Sort == LVS_ALIAS_DESC
			return lstrcmpiW((*pOpcodeInfo)[(WORD)lParam2].wszAlias, (*pOpcodeInfo)[(WORD)lParam1].wszAlias);
	}
	...
	else if (Sort == LVS_COMMENT_ASC || Sort == LVS_COMMENT_DESC)
	{
		...
		if (Sort == LVS_COMMENT_ASC)
			return lstrcmpiW((*pOpcodeInfo)[(WORD)lParam1].wszComment, (*pOpcodeInfo)[(WORD)lParam2].wszComment);
		else // Sort == LVS_COMMENT_DESC
			return lstrcmpiW((*pOpcodeInfo)[(WORD)lParam2].wszComment, (*pOpcodeInfo)[(WORD)lParam1].wszComment);
	}
...
VOID CSettingsPacket::ListviewRightClick(__in LPNMITEMACTIVATE lpnmia)
{
	...
	while ((lvi.iItem = ListView_GetNextItem(hListview, lvi.iItem, LVNI_SELECTED)) != -1)
	{
		...
		vwSelected.push_back((WORD)lvi.lParam);
		if (!(*pOpcodeInfo)[(WORD)lvi.lParam].bBlock)
			bAllBlocked = FALSE;
		if (!(*pOpcodeInfo)[(WORD)lvi.lParam].bIgnore)
			bAllIgnored = FALSE;
	}
	...
	if (vwSelected.size() == 1)
		mii.fState = (*pOpcodeInfo)[vwSelected.at(0)].bBlock ? MFS_DISABLED : (MFS_ENABLED | ((*pOpcodeInfo)[vwSelected.at(0)].bIgnore ? MFS_CHECKED : MFS_UNCHECKED));
	else
	...
	if (vwSelected.size() == 1)
		mii.fState = MFS_ENABLED | ((*pOpcodeInfo)[vwSelected.at(0)].bBlock ? MFS_CHECKED : MFS_UNCHECKED);
	else
	...
	if (GetCursorPos(&pt))
	{
		...
		case IDM_IGNORE:
			...
			{
				(*pOpcodeInfo)[vwSelected.at(0)].bIgnore = !(*pOpcodeInfo)[vwSelected.at(0)].bIgnore;
				OnUpdateOpcode(vwSelected.at(0));
			}
			break;
		...
		case IDM_BLOCK:
			if (vwSelected.size() > 1)
			{
				foreach(WORD& wOpcode, vwSelected)
				{
					...
					if (!bAllBlocked)
						(*pOpcodeInfo)[wOpcode].bIgnore = FALSE;
					OnUpdateOpcode(wOpcode);
				}
			}
			else
			{
				if (!((*pOpcodeInfo)[vwSelected.at(0)].bBlock = !(*pOpcodeInfo)[vwSelected.at(0)].bBlock))
					(*pOpcodeInfo)[vwSelected.at(0)].bIgnore = FALSE;
				else
					(*pOpcodeInfo)[vwSelected.at(0)].bIgnore = TRUE;
				OnUpdateOpcode(vwSelected.at(0));
			}
			break;
...
INT_PTR CALLBACK CSettingsPacket::DialogProc(__in HWND hDialog, __in UINT uMessage, __in WPARAM wParam, __in LPARAM lParam)
{
	...
	case WM_UPDATEOPCODE:
		GetClassInstance<CSettingsPacket>(hDialog)->OnUpdateOpcode((WORD)wParam);
		break;
