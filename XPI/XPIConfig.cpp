...
BOOL ParseOpcode(__in IXmlReader* pReader)
{
	...
	wOpcode = (WORD)wcstoul(lpcwszValue, &lpwszEndPtr, 10);
	...
	while (pReader->Read(&NodeType) == S_OK)
	{
		...
		else if (NodeType == XmlNodeType_Text)
		{
			...
			{
				if (lstrcmpiW(lpcwszValue, L"true") == 0)
					(*pOpcodeInfo)[wOpcode].bBlock = TRUE;
				else
					(*pOpcodeInfo)[wOpcode].bBlock = FALSE;
			}
			else if (lstrcmpiW(lpcwszElement, L"alias") == 0)
			{
				StringCchCopyW((*pOpcodeInfo)[wOpcode].wszAlias, _countof((*pOpcodeInfo)[wOpcode].wszAlias), lpcwszValue);
			}
			else if (lstrcmpiW(lpcwszElement, L"comment") == 0)
			{
				StringCchCopyW((*pOpcodeInfo)[wOpcode].wszComment, _countof((*pOpcodeInfo)[wOpcode].wszComment), lpcwszValue);
			}
			else if (lstrcmpiW(lpcwszElement, L"color") == 0)
			{
				for (INT i = 0; i < COUNT_XPI_COLOR; i++)
				{
					...
					{
						(*pOpcodeInfo)[wOpcode].crColor = g_XPIColors[i].crColor;
						break;
					}
				}
			}
...
VOID WriteOpcodeEntry(__in IXmlWriter* pWriter, __in WORD wOpcode, __in BOOL bIgnore, __in BOOL bBlock, __in_z_opt LPCWSTR lpcwszAlias, __in_z_opt LPCWSTR lpcwszComment, __in_opt COLORREF crColor = 0)
{
	if (pWriter == NULL)
		return;
	...
	StringCchPrintfW(wszTemp, _countof(wszTemp), L"%d", wOpcode);
