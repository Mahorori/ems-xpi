...
VOID CFormattedView::CopyPackets(__in BOOL bHeaders)
{
	...
	foreach(CMaplePacket* pPacket, vPackets)
	{
		...
		if (bHeaders) // show packet headers (direction + size)
		{
			...
			if (pOpcodeInfo != NULL)
			{
				if ((*pOpcodeInfo)[wOpcode].wszAlias[0] == 0)
					StringCchPrintfA(szTemp, _countof(szTemp), "%04X", wOpcode);
				else
					WideCharToMultiByte(CP_ACP, 0, (*pOpcodeInfo)[wOpcode].wszAlias, -1, szTemp, _countof(szTemp), NULL, NULL);
			}
...
VOID CFormattedView::OnAddPacket(__inout CMaplePacket* pPacket)
{
	...
	{
		StringCchPrintfW(wszTemp, _countof(wszTemp), L"%04X", pPacket->GetOpcode());
		lvi.pszText = wszTemp;
	}
	else
...
VOID CFormattedView::ListviewDoubleClick(__in LPNMITEMACTIVATE lpnmia)
{
	...
	else
	{
		...
		wstrText += L": ";
		WORD  wOpcode = pPacket->GetOpcode();
		WCHAR wszTemp[48];

		StringCchPrintfW(wszTemp, _countof(wszTemp), L"0x%04X", wOpcode);
		wstrText += wszTemp;

		if ((*pOpcodeInfo)[wOpcode].wszAlias[0] != 0)
		{
			StringCchPrintfW(wszTemp, _countof(wszTemp), L" <%s>", (*pOpcodeInfo)[wOpcode].wszAlias);
			wstrText += wszTemp;
		}
		...
		if (FileTimeToSystemTime(&pPacket->GetCallerTime()->Time, &SystemTime))
		{
			...
			if ((*pOpcodeInfo)[wOpcode].wszComment[0] != 0)
			{
				wstrText += L"\r\n";
				wstrText += pStrings->Get(IDS_COMMENT);
				wstrText += L": ";
				wstrText += (*pOpcodeInfo)[wOpcode].wszComment;
			}
		}
...
VOID CFormattedView::ListviewRightClick(__in LPNMITEMACTIVATE lpnmia)
{
	...
	while ((lvi.iItem = ListView_GetNextItem(hListview, lvi.iItem, LVNI_SELECTED)) != -1)
	{
		...
		if (pPacket != NULL)
		{
			...
			{
				wOpcode = pPacket->GetOpcode();
				pTempOpcode = &(*pOpcodeInfo)[wOpcode];
				if (pTempOpcode == NULL)
					break;
			}
		}
	...
	if (GetCursorPos(&pt))
	{
		...
		case IDM_IGNORE:
			...
			{
				(*pOpcodeInfo)[wOpcode].bIgnore = !(*pOpcodeInfo)[wOpcode].bIgnore;
				SendMessage(GetParent(m_hDialog), WM_UPDATEOPCODE, wOpcode, 0);
			}
			break;
		...
		case IDM_BLOCK:
			if (swSelected.size() > 1)
			{
				foreach(WORD wTemp, swSelected)
				{
					...
					if (!bAllBlocked)
						(*pOpcodeInfo)[wTemp].bIgnore = FALSE;
					SendMessage(GetParent(m_hDialog), WM_UPDATEOPCODE, wTemp, 0);
				}
			}
			else
			{
				if (!((*pOpcodeInfo)[wOpcode].bBlock = !(*pOpcodeInfo)[wOpcode].bBlock))
					(*pOpcodeInfo)[wOpcode].bIgnore = FALSE;
				else
					(*pOpcodeInfo)[wOpcode].bIgnore = TRUE;
...
VOID CFormattedView::UpdateOpcodeAlias(__in WORD wOpcode, __in LPWSTR lpwszBuffer)
{
	HWND hListview = GetDlgItem(m_hDialog, IDC_FORMATTEDLIST);
	if (hListview == NULL)
		return;
	...
	for (lvi.iItem; lvi.iItem < ListView_GetItemCount(hListview); lvi.iItem++)
	{
		...
		if (pPacket->GetMemberCount() < 1)
			continue;
		if (pPacket->GetOpcode() == wOpcode)
		{
			if (lpwszBuffer[0] == 0)
			{
				StringCchPrintfW(lpwszBuffer, OP_ALIAS_MAXCC, L"%04X", wOpcode);
...
VOID CFormattedView::ListviewEndEdit(__in HWND* hEdit, __in NMLVDISPINFO* lpnmlvdi)
{
	...
	WCHAR *lpwszBuffer = (*pOpcodeInfo)[wOpcode].wszAlias;
	...
	UpdateOpcodeAlias(wOpcode, lpwszBuffer);

	PostMessage(GetParent(m_hDialog), WM_UPDATEOPCODE, wOpcode, 0);
